...
use wgpu_3dgs_viewer::{
    CameraPod, DepthSorter, LampshadeSorter, Viewer, ViewerCreateDepthSorterFactoryContext,
    ViewerCreateOptions,
    core::{
...
fn embedded_viewer(ctx: &TestContext, gaussians: &impl IterGaussian) -> Viewer<G> {
    let mut viewer =
...
) -> Viewer<G, LampshadeSorter> {
    let mut viewer = Viewer::<G, LampshadeSorter>::new_with_options(
        &ctx.device,
        ...
        ViewerCreateOptions {
            ...
            cache: None,
            depth_sorter_factory: |ctx: ViewerCreateDepthSorterFactoryContext<G>| {
                LampshadeSorter::new(ctx)
            },
...
fn render_pixels<S: DepthSorter>(ctx: &TestContext, viewer: &Viewer<G, S>) -> (Vec<u8>, u32) {
...
fn test_lampshade_render_matches_embedded_as_visibility_changes() {
    ...
    let mut accelerated = accelerated_viewer(&ctx, &gaussians);
    assert!(accelerated.depth_sorter.is_accelerated());
    let mut embedded = embedded_viewer(&ctx, &gaussians);
    ...
    for shift in [0.0, -2.0, -10.0, 0.0] {
        accelerated.update_model_transform(&ctx.queue, Vec3::Z * shift, Quat::IDENTITY, Vec3::ONE);
        embedded.update_model_transform(&ctx.queue, Vec3::Z * shift, Quat::IDENTITY, Vec3::ONE);
...
fn test_lampshade_falls_back_without_enabled_subgroups() {
    ...
    let fallback = accelerated_viewer(&ctx, &scene(1));
    assert!(!fallback.depth_sorter.is_accelerated());
