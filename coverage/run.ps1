...
cargo llvm-cov run --example simple --all-features -- -m "$BASE_DIR/model.ply"

echo "Running 'multi-model' example"
cargo llvm-cov run --example multi-model --all-features -- -m "$BASE_DIR/model.ply" -m "$BASE_DIR/model.ply"

echo "Running 'selection' example"
cargo llvm-cov run --example selection --all-features -- -m "$BASE_DIR/model.ply"
